...
sub list_all {
    my ($self) = @_;
    return $self->db->query('SELECT * FROM products ORDER BY name')->hashes;
}

sub find {
    my ($self, $id) = @_;
    return $self->db->query('SELECT * FROM products WHERE id = $1', $id)->hash;
}
...
sub update_fields {
    ...
    return $self->db->query("UPDATE products SET $set WHERE id = \$$i RETURNING *", @vals)->hash;
}
